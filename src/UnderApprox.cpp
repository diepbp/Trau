			...
			if (element.second.compare(FALSETR) == 0){
				...
				if (value02.compare(NOTFOUND) != 0 && value01.compare(NOTFOUND) == 0 &&
					...
					global_smtStatements.push_back({create_constraints_NOTContain(element.first.args[0].name, value02)});
						...
						done.find(std::make_pair(value01, element.first.args[1].name)) == done.end()) {
...
void handle_NOTEqual(
	...
	for (const auto& s : rewriterStrMap) {
		if (s.first.name.compare("=") == 0){
			if (s.first.args[0].args.size() == 0 && s.first.args[1].args.size() == 0 && s.second.compare(FALSETR) == 0)  {
				if ((connectedVariables.find(s.first.args[0].name) != connectedVariables.end() ||
						connectedVariables.find(s.first.args[1].name) != connectedVariables.end() ||
						s.first.args[0].name.compare("\"\"") == 0 ||
						s.first.args[1].name.compare("\"\"") == 0)){
					global_smtStatements.push_back({create_constraints_NOTEqual(s.first.args[0].name, s.first.args[1].name)});
				}
				else if (s.first.args[0].name[0] != '"' && s.first.args[1].name[0] != '"'){
					global_smtStatements.push_back({createNotOperator(createEqualConstraint(generateVarLength(s.first.args[0].name), generateVarLength(s.first.args[1].name)))});
				}
			}
		}
	}
...
void create_constraints_const(std::vector<std::string> &defines, std::vector<std::string> &constraints){
	...
	for (const auto constNode : constMap)
		if (!isRegexStr(constNode.first) && isUnionStr(constNode.first)){
			...
			std::string lenName = std::string(LENPREFIX) + constNode.second + REGEXSUFFIX;
			defines.emplace_back(createIntDefinition(lenName));
...
void create_constraints_regex(std::vector<std::string> &defines, std::vector<std::string> &constraints, std::string regex, std::string name){
	...
	int cnt = 0;
	std::string constraint = "(assert (= " + std::string(LENPREFIX) + name + std::string(REGEXSUFFIX) + " (+ ";
...
std::string createLengthConstraintForAssignment(std::string x, std::vector<std::string> components){
	...
	for (const auto& component: components){
		if (component[0] != '\"'){ /* not a const */
			...
			if(!isRegexStr(component)) {
				...
				lenX = lenX + " " + LENPREFIX + constMap[component.substr(1, component.length() - 2)] + REGEXSUFFIX;
				cnt ++;
			}
		}
		else {
	...
	if (x[0] != '\"')
		...
		if(!isRegexStr(x) && !isUnionStr(x))
			...
			lenX = createEqualConstraint(lenX, LENPREFIX + constMap[x.substr(1, x.length() - 2)] + std::string(REGEXSUFFIX));
...
//			__debugPrint(logFile, "%d %s -> %s -- %s\n", __LINE__, op.toString().c_str(), s.first.arg01.c_str(), s.first.arg02.c_str());
			...
			if (s.first.name.compare("=") == 0 && s.second.compare(FALSETR) == 0) {
				if (s.first.args[0].name.compare("\"\"") == 0 || s.first.args[1].name.compare("\"\"") == 0)
					continue;

				bool canSkip = false;
				if (s.first.args[1].name[0] == '"' && !isRegexStr(s.first.args[1].name)) {
					if (equalitiesMap.find(s.first.args[0].name) != equalitiesMap.end())
						for (const auto _eq : equalitiesMap[s.first.args[0].name]) {
							for (const auto e : _eq)
								if (e[0] == '"' && !isRegexStr(e) && !isUnionStr(e) && s.first.args[1].name.find(e) == std::string::npos) {
									canSkip = true;
									break;
								}
							if (canSkip)
				...
				if (s.first.args[0].name[0] == '"' && !isRegexStr(s.first.args[0].name)) {
					if (equalitiesMap.find(s.first.args[1].name) != equalitiesMap.end())
						for (const auto _eq : equalitiesMap[s.first.args[1].name]) {
							for (const auto e : _eq)
								if (e[0] == '"' && !isRegexStr(e) && !isUnionStr(e) && s.first.args[0].name.find(e) == std::string::npos) {
									canSkip = true;
									break;
								}
							if (canSkip)
				...
				if (canSkip) {
					__debugPrint(logFile, "%d skip %s != %s\n", __LINE__, s.first.args[0].name.c_str(), s.first.args[1].name.c_str());
					continue;
				}
			}
...
void formatOtherVars(
	...
	for (const auto& op : orgRewriterStrMap){
		if (op.first.name.compare("=") == 0 &&
				...
				op.first.args[1].name.find("\"") == std::string::npos &&
...
void formatConnectedVars(
	...
	for (const auto& s : indexes) {
		...
		if (lenVector[s].second == 0) {
			...
			if (varName.length() > std::string(REGEXSUFFIX).length() &&
					varName.substr(varName.length() - std::string(REGEXSUFFIX).length()).compare(REGEXSUFFIX) == 0)
				varName = varName.substr(0, varName.length() - std::string(REGEXSUFFIX).length());
			std::string orgVarName = varName;
...
std::map<std::string, std::string> formatResult(
	...
	for (const auto& s : _strValue)
		if (s.first.find(ARRPREFIX) == 0){
			std::string name = s.first.substr(std::string(ARRPREFIX).length());
			...
			if (fgets(buffer, 9900, in) != NULL) {
				...
				if (line.find("(define") != std::string::npos) {
					...
					if (array_map.find(name) != array_map.end()){
						...
						std::string valueStr = "";
						for (unsigned j = 0; j < valueMap.size(); ++ j) {
...
std::map<std::string, std::vector<std::string>> createNotEqualMap(std::map<StringOP, std::string> rewriterStrMap){
	...
	for (const auto op : rewriterStrMap)
		if (op.first.name.compare("=") == 0 && op.second.compare(FALSETR) == 0){
			...
			if (arg02[0] == '"') {
				std::string arg02 = op.first.args[1].name;
				if (ret.find(op.first.args[0].toString()) == ret.end())
					ret[op.first.args[0].toString()] = {arg02.substr(1, arg02.length() - 2)};
				else
					ret[op.first.args[0].toString()].emplace_back(arg02.substr(1, arg02.length() - 2));
			}
		}
	return ret;
}
...
std::vector<std::string> removeConnectedVarsIfNotInEqualities(){
	...
	for (const auto& var : connectedVariables) {
		...
		if (found)
			...
			removedConnectedVars.emplace_back(var.first);
		}
	}

	connectedVariables.clear();
	connectedVariables = newConnectedVars;
	return removedConnectedVars;
}
...
void updateRewriter(
	...
	for (const auto& op : rewriterStrMap){
		...
		if (op.first.name.compare("=") == 0 && op.second.compare(FALSETR) == 0){
			if (!isTrivialInequality(op.first.args[0].toString(), op.first.args[1].toString())) {
				newRewriterStrMap[op.first] = op.second;
			}
			continue;
		}
