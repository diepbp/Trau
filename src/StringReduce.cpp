...
std::string exportNodeName(Z3_theory t,  Z3_ast const args[], Z3_func_decl name) {
	...
	if (name == td->Concat)
		return "("  + std::string(languageMap[CONCAT]) + " " + node01 + " " + node02 + ")";
	if (name == td->Contains)
		return "("  + std::string(languageMap[CONTAINS]) + " " + node01 + " " + node02 + ")";
	else if (name == td->LastIndexof)
		return "("  + std::string(languageMap[LASTINDEXOF]) + " " + node01 + " " + node02 + ")";
	else if (name == td->Indexof)
		return "("  + std::string(languageMap[INDEXOF]) + " " + node01 + " " + node02 + ")";
	else if (name == td->StartsWith)
		return "("  + std::string(languageMap[STARTSWITH]) + " " + node01 + " " + node02 + ")";
	else if (name == td->EndsWith)
		return "("  + std::string(languageMap[ENDSWITH]) + " " + node01 + " " + node02 + ")";
	else if (name == td->Replace)
		return "("  + std::string(languageMap[REPLACE]) + " " + node01 + " " + node02 + " " + node03 + ")";
	else if (name == td->ReplaceAll)
		return "("  + std::string(languageMap[REPLACEALL]) + " " + node01 + " " + node02 + " " + node03 + ")";
	else if (name == td->SubString)
		return "("  + std::string(languageMap[SUBSTRING]) + " " + node01 + " " + node02 + " " + node03 + ")";
	else {
		assert(false);
		return NULL;
	}
...
std::string exportNodeName(Z3_theory t,  Z3_ast node){
	...
	if (isDetAutomatonFunc(t, node) || isConstStr(t, node) || isNonDetAutomatonFunc(t, node)) {
		...
		return "(" + std::string(languageMap[CONCAT]) + " " + exportNodeName(t, Z3_get_app_arg(ctx, Z3_to_app(ctx, node), 0)) + " " +
...
Z3_ast reduce_replaceAll(Z3_theory t, Z3_ast const args[], Z3_ast & breakdownAssert) {
	...
	if (getNodeType(t, args[0]) == my_Z3_ConstStr && getNodeType(t, args[1]) == my_Z3_ConstStr && getNodeType(t, args[2]) == my_Z3_ConstStr) {
		...
		if (arg0Str.find(arg1Str) != std::string::npos) {
			...
			replaceAllStrMap[StringOP(languageMap[REPLACEALL], exportNodeName(t, args[0]), exportNodeName(t, args[1]), exportNodeName(t, args[2]))] = TRUESTR;
			return mk_str_value(t, arg0Str.c_str());
		} else {
			replaceAllStrMap[StringOP(languageMap[REPLACEALL], exportNodeName(t, args[0]), exportNodeName(t, args[1]), exportNodeName(t, args[2]))] = FALSETR;
			return args[0];
		}
	} else {
		AutomatonStringData * td = (AutomatonStringData*) Z3_theory_get_ext_data(t);
		...
		replaceAllStrMap[StringOP(languageMap[REPLACEALL], exportNodeName(t, args[0]), exportNodeName(t, args[1]), exportNodeName(t, args[2]))] = boolName;
...
//		pushVectors(Z3_mk_implies(ctx, condAst_x2, condAst_arg00), thenItems00, thenItems01, thenItems02);
		generalConstraints.push_back(Z3_mk_implies(ctx, condAst_x2, condAst_arg00));
...
//			pushVectors(Z3_mk_implies(ctx, condAst00n, condAst_arg00), thenItems00, thenItems01, thenItems02);
			generalConstraints.push_back(Z3_mk_implies(ctx, condAst_x1, condAst00n));
			generalConstraints.push_back(Z3_mk_implies(ctx, condAst00n, condAst_arg00));
...
//			pushVectors(Z3_mk_implies(ctx, condAst_x1, condAst_arg00), thenItems00, thenItems01, thenItems02);
			generalConstraints.push_back(Z3_mk_implies(ctx, condAst_x1, condAst_arg00));
		...
		generalConstraints.push_back(Z3_mk_implies(ctx, condAst_x4, condAst_x2));
...
//			pushVectors(Z3_mk_implies(ctx, condAst_x3, condAst02n), thenItems01, thenItems02);
			generalConstraints.push_back(Z3_mk_implies(ctx, condAst02n, condAst_x2));
			generalConstraints.push_back(Z3_mk_implies(ctx, condAst_x3, condAst02n));
...
//			pushVectors(Z3_mk_implies(ctx, condAst_x3, condAst_x2), thenItems01, thenItems02);
			generalConstraints.push_back(Z3_mk_implies(ctx, condAst_x3, condAst_x2));
		...
		generalConstraints.push_back(Z3_mk_implies(ctx, condAst_x6, condAst_x4));
...
//			thenItems02.push_back(Z3_mk_implies(ctx, condAst04n, condAst_x4));
			generalConstraints.push_back(Z3_mk_implies(ctx, condAst_x5, condAst04n));
			generalConstraints.push_back(Z3_mk_implies(ctx, condAst04n, condAst_x4));
...
//			thenItems02.push_back(Z3_mk_implies(ctx, condAst_x6, condAst_x4));
			generalConstraints.push_back(Z3_mk_implies(ctx, condAst_x5, condAst_x4));
		...
		Z3_ast breakdown[2] = {mk_and_fromVector(t, generalConstraints), Z3_mk_ite(ctx, condAst_arg00, ite02, elseBranch)};
...
Z3_ast reduce_replace(Z3_theory t, Z3_ast const args[], Z3_ast & breakdownAssert) {
	...
	if (getNodeType(t, args[0]) == my_Z3_ConstStr && getNodeType(t, args[1]) == my_Z3_ConstStr && getNodeType(t, args[2]) == my_Z3_ConstStr) {
		...
		if (arg0Str.find(arg1Str) != std::string::npos) {
			...
			replaceStrMap[StringOP(languageMap[REPLACE], exportNodeName(t, args[0]), exportNodeName(t, args[1]), exportNodeName(t, args[2]))] = TRUESTR;
			return mk_str_value(t, replaced.c_str());
		} else {
			replaceStrMap[StringOP(languageMap[REPLACE], exportNodeName(t, args[0]), exportNodeName(t, args[1]), exportNodeName(t, args[2]))] = FALSETR;
			return args[0];
		}
	} else {
		...
		replaceStrMap[StringOP(languageMap[REPLACE], exportNodeName(t, args[0]), exportNodeName(t, args[1]), exportNodeName(t, args[2]))] = boolName;
		...
		if (printingConstraints)
			constraintSet.stringConstraints.emplace(node_to_string(t, thenItems[thenItems.size() - 1]));

		Z3_ast condAst_x1 = registerContain(t, x1, args[1]);
		thenItems.push_back(Z3_mk_not(ctx, condAst_x1));
		if (printingConstraints)
			constraintSet.otherConstraints.emplace(node_to_string(t, thenItems[thenItems.size() - 1]));
		...
		if (!canSkipExt(t, args[1])) {
			...
			if (printingConstraints)
				constraintSet.arithmeticConstraints.emplace(node_to_string(t, thenItems[thenItems.size() - 1]));
		...
		if (printingConstraints) {
			constraintSet.arithmeticConstraints.emplace(node_to_string(t, elseBranch));
			constraintSet.otherConstraints.emplace(node_to_string(t, breakdownAssert));
		}
...
Z3_ast reduce_contains(Z3_theory t, Z3_ast const args[], Z3_ast & breakdownAssert) {
	...
	if (getNodeType(t, args[0]) == my_Z3_ConstStr && getNodeType(t, args[1]) == my_Z3_ConstStr) {
		...
		bool update = true;
		Z3_ast breakdown = Z3_mk_eq(ctx, args[0], mk_concat(t, ts0, mk_concat(t, args[1], ts1, update), update));
		breakdownAssert = Z3_mk_eq(ctx, reduceAst, breakdown);
		if (printingConstraints) {
			constraintSet.stringConstraints.emplace(node_to_string(t, breakdown));
			constraintSet.otherConstraints.emplace(node_to_string(t, breakdownAssert));
		}
	}
	...
	if (getNodeType(t, args[0]) == my_Z3_ConstStr && getNodeType(t, args[1]) == my_Z3_ConstStr) {
		...
		if (arg0Str.length() < arg1Str.length()) {
			...
			if (arg0Str.substr(0, arg1Str.length()) == arg1Str) {
				startsWithStrMap[StringOP(languageMap[STARTSWITH], exportNodeName(t, args[0]), exportNodeName(t, args[1]))] = TRUESTR;
				reduceAst = Z3_mk_true(ctx);
			} else {
				startsWithStrMap[StringOP(languageMap[STARTSWITH], exportNodeName(t, args[0]), exportNodeName(t, args[1]))] = FALSETR;
				reduceAst = Z3_mk_false(ctx);
			}
		}
	} else {
		...
		startsWithStrMap[StringOP(languageMap[STARTSWITH], exportNodeName(t, args[0]), exportNodeName(t, args[1]))] = boolVar;
		...
		if (printingConstraints)
			constraintSet.stringConstraints.emplace(node_to_string(t, ands[ands.size() - 1]));

		ands.push_back( Z3_mk_eq(ctx, mk_length(t, ts0), mk_length(t, args[1])) );
		if (printingConstraints)
			constraintSet.arithmeticConstraints.emplace(node_to_string(t, ands[ands.size() - 1]));

		ands.push_back( Z3_mk_ite(ctx, Z3_mk_eq(ctx, ts0, args[1]), Z3_mk_eq(ctx, resBoolVar, Z3_mk_true(ctx)), Z3_mk_eq(ctx, resBoolVar, Z3_mk_false(ctx) ) ) );
		if (printingConstraints){
			constraintSet.otherConstraints.emplace(node_to_string(t, ands[ands.size() - 1]));
			constraintSet.stringConstraints.emplace(node_to_string(t, Z3_mk_eq(ctx, ts0, args[1])));
			constraintSet.otherConstraints.emplace(node_to_string(t, Z3_mk_eq(ctx, resBoolVar, Z3_mk_true(ctx))));
			constraintSet.otherConstraints.emplace(node_to_string(t, Z3_mk_eq(ctx, resBoolVar, Z3_mk_false(ctx))));
		}
		...
		if (printingConstraints){
			constraintSet.arithmeticConstraints.emplace(node_to_string(t, Z3_mk_ge(ctx, mk_length(t, args[0]), mk_length(t, args[1]))));
			constraintSet.otherConstraints.emplace(node_to_string(t, breakdownAssert));
		}
	...
	if (getNodeType(t, args[0]) == my_Z3_ConstStr && getNodeType(t, args[1]) == my_Z3_ConstStr) {
		...
		if (arg0Str.length() < arg1Str.length()) {
			...
			if (arg0Str.substr(arg0Str.length() - arg1Str.length(), arg1Str.length()) == arg1Str) {
				endsWithStrMap[StringOP(languageMap[ENDSWITH], exportNodeName(t, args[0]), exportNodeName(t, args[1]))] = TRUESTR;
				reduceAst = Z3_mk_true(ctx);
			} else {
				endsWithStrMap[StringOP(languageMap[ENDSWITH], exportNodeName(t, args[0]), exportNodeName(t, args[1]))] = FALSETR;
				reduceAst = Z3_mk_false(ctx);
			}
		}
	} else {
		...
		Z3_ast ts1 = mk_internal_string_var(t);
		std::string boolVar = Z3_ast_to_string(ctx, resBoolVar);
		endsWithStrMap[StringOP(languageMap[ENDSWITH], exportNodeName(t, args[0]), exportNodeName(t, args[1]))] = boolVar;
		...
		ands.push_back( Z3_mk_ite(ctx, Z3_mk_eq(ctx, ts1, args[1]), Z3_mk_eq(ctx, resBoolVar, Z3_mk_true(ctx)), Z3_mk_eq(ctx, resBoolVar, Z3_mk_false(ctx) ) ) );
		if (printingConstraints){
			constraintSet.otherConstraints.emplace(node_to_string(t, ands[ands.size() - 1]));
			constraintSet.stringConstraints.emplace(node_to_string(t, Z3_mk_eq(ctx, ts1, args[1])));
			constraintSet.otherConstraints.emplace(node_to_string(t, Z3_mk_eq(ctx, resBoolVar, Z3_mk_true(ctx))));
			constraintSet.otherConstraints.emplace(node_to_string(t, Z3_mk_eq(ctx, resBoolVar, Z3_mk_false(ctx))));
		}
...
Z3_ast reduce_indexof(Z3_theory t, Z3_ast const args[], Z3_ast & breakdownAssert) {
	...
	if (getNodeType(t, args[0]) == my_Z3_ConstStr && getNodeType(t, args[1]) == my_Z3_ConstStr) {
		...
		if (arg0Str.find(arg1Str) != std::string::npos) {
			...
			indexOfStrMap[StringOP(languageMap[INDEXOF], exportNodeName(t, args[0]), exportNodeName(t, args[1]))] = std::make_pair("", std::to_string(index));
			return mk_int(ctx, index);
		} else {
			indexOfStrMap[StringOP(languageMap[INDEXOF], exportNodeName(t, args[0]), exportNodeName(t, args[1]))] = std::make_pair("", "-1");
			return mk_int(ctx, -1);
		}
	} else {
		/* check if the contraint is handled */
		...
		indexOfStrMap[StringOP(languageMap[INDEXOF], exportNodeName(t, args[0]), exportNodeName(t, args[1]))] = std::make_pair(boolVar, LENPREFIX + std::string(Z3_ast_to_string(ctx, x1)));
...
Z3_ast reduce_lastindexof(Z3_theory t, Z3_ast const args[], Z3_ast & breakdownAssert) {
	...
	if (getNodeType(t, args[0]) == my_Z3_ConstStr && getNodeType(t, args[1]) == my_Z3_ConstStr) {
		...
		if (arg0Str.rfind(arg1Str) != std::string::npos) {
			...
			lastIndexOfStrMap[StringOP(languageMap[LASTINDEXOF], exportNodeName(t, args[0]), exportNodeName(t, args[1]))] = std::make_pair("", std::to_string(index));
			return mk_int(ctx, index);
		} else {
			lastIndexOfStrMap[StringOP(languageMap[LASTINDEXOF], exportNodeName(t, args[0]), exportNodeName(t, args[1]))] = std::make_pair("", "-1");
			return mk_int(ctx, -1);
		}
	} else {
		std::vector<Z3_ast> tmpInternalVars;
		...
		lastIndexOfStrMap[StringOP(languageMap[LASTINDEXOF], exportNodeName(t, args[0]), exportNodeName(t, args[1]))] = std::make_pair(boolVar, LENPREFIX + std::string(Z3_ast_to_string(ctx, x1)));
		...
		breakdownAssert = Z3_mk_ite(ctx, condAst_arg0, mk_and_fromVector(t, thenItems), elseItems);
		if (printingConstraints){
			constraintSet.arithmeticConstraints.emplace(node_to_string(t, elseItems));
			constraintSet.otherConstraints.emplace(node_to_string(t, breakdownAssert));
		}
		return indexAst;
	}
...
Z3_ast reduce_charAt(Z3_theory t, Z3_ast const args[], Z3_ast & breakdownAssert) {
	...
	if (getNodeType(t, args[0]) == my_Z3_ConstStr && getNodeType(t, args[1]) == my_Z3_Num) {
		...
		bool update;
		Z3_ast ands[3];
		ands[0] = Z3_mk_eq(ctx, args[0], mk_concat(t, ts0, mk_concat(t, ts1, ts2, update), update));
		...
		charAtStrMap[StringOP(languageMap[CHARAT], exportNodeName(t, args[0]), exportNodeName(t, args[1]))] = std::make_pair(boolVar, LENPREFIX + std::string(Z3_ast_to_string(ctx, ts0)));

		if (printingConstraints){
			constraintSet.stringConstraints.emplace(node_to_string(t, ands[0]));
			constraintSet.arithmeticConstraints.emplace(node_to_string(t, ands[1]));
			constraintSet.arithmeticConstraints.emplace(node_to_string(t, ands[2]));
			constraintSet.stringConstraints.emplace(node_to_string(t, elseBranch));
...
Z3_ast reduce_subStr(Z3_theory t, Z3_ast const args[], Z3_ast & breakdownAssert) {
	...
	if (value_prefix != 0 ) {
		...
		std::string tmp = createEqualConstraint(std::string(LENPREFIX) + (std::string)Z3_ast_to_string(ctx, ts0), Z3_ast_to_string(ctx, args[1]));

		subStrStrMap[StringOP(languageMap[SUBSTRING], exportNodeName(t, args[0]), exportNodeName(t, args[1]), exportNodeName(t, args[2]))] = tmp;
		...
		subStrStrMap[StringOP(languageMap[SUBSTRING], exportNodeName(t, args[0]), exportNodeName(t, args[1]), exportNodeName(t, args[2]))] = TRUESTR;
...
Z3_bool cb_reduce_eq(Z3_theory t, Z3_ast s1, Z3_ast s2, Z3_ast * r) {
	Z3_context ctx = Z3_theory_get_context(t);
	/* write to file */
	std::string tmp = createEqualConstraint(exportNodeName(t, s1), exportNodeName(t, s2));
	if (printingConstraints)
