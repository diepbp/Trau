...
std::vector<char*> TokenParser::stringTokens(char* line){
 ...
 */
bool TokenParser::readAutomata(std::string fileName){
    FILE* fp;
...
std::string TokenParser::findType(char ch){ //-------------------------CHANGE-----------------
	std::string tmp = "";
 ...
 */
int TokenParser::processError(int start, std::string error, std::string line){
...
std::vector<TokenElement> TokenParser::tokenHandler(std::string s, int lineNo){
...
void TokenParser::readCFile(std::string fileName){
	FILE* fp;
	const int SIZEBUF = 2048;
	char buf [SIZEBUF];
